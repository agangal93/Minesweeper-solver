...
class BoardDesign:
    ...
    def ExploreAllOpenBlocks(self,node,KB,explored_cells):
        ...
        while not Node_queue.empty():
            ...
            if indicator == 0:
                for cell in neighbour:
                    if not self.cell_array[cell].Cell_isexplored() and not self.cell_array[cell].Cell_IsFlaggedMine():
    ...
    def StartExploration(self,start):
        ...
        return hit_mine, len(explored_cells), explored_cells, len(KB.mine_cells)
...
num_mines_range = [5,10,15,20,25]
avg_score = []
for num_mines in num_mines_range:
    print("Mine density - %d"%num_mines)
    mine_cells = []
    for i in range(0,100):
        ...
        start_point = 0
        hit_mine, exploration_count, explored_cells, num_mine_cells = B.StartExploration(start_point)

        mine_cells.append(num_mine_cells/num_mines)
        #if not hit_mine:
        #    passed_count += 1
        # Display exploration path
        #print("Explored cells")
        #print(explored_cells)
        print("no of mine cells - %d Exploration count = %d"%(num_mine_cells,exploration_count))
        #print("Score - %f"%(mines_hit/num_mines))

    print("Mine density - %d"%num_mines)
    avg_score.append(mean(mine_cells))

#master.mainloop()
print("Score vs mine density")
print(avg_score)

plt.plot(num_mines_range,avg_score)
plt.title("Score vs mine density")
